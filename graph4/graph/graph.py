...
class Graph(GraphBase):
    ...
    def get_all_information(self, density_matrix:np.ndarray):
        ...
        """
        cars = np.sum(density_matrix)
        people = np.sum(density_matrix)
        crowding = np.sum(density_matrix)
    ...
    def get_point_information(self, point_id: int, density_matrix:np.ndarray):
        ...
        cars = np.sum(density_matrix[point_id])
        people = np.sum(density_matrix[point_id])
        crowding = np.sum(density_matrix[point_id])
        emergency = np.sum(density_matrix[point_id])
        trafficLight = self.now_light(-1,point_id)
        return [cars, people, crowding, emergency, trafficLight]
    ...
    def get_road_information(self, road_id: int, density_matrix:np.ndarray):
        ...
        :return:[cars, people, crowding, emergency, trafficLight]
        """
        cars = np.sum(density_matrix[road_id])
        people = np.sum(density_matrix[road_id])
        crowding = np.sum(density_matrix[road_id])
        emergency = np.sum(density_matrix[road_id])
        trafficLight = self.now_light(road_id)
        return [cars, people, crowding, emergency, trafficLight]
